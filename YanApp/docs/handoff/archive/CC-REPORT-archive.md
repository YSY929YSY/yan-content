# CC-REPORT 归档（2026-08-26 之前）

> 从 `docs/handoff/CC-REPORT.md` 切出来的历史记录。**不要读整份。**
> 需要哪一节就 `grep -n '^## ' 本文件` 找到行号，再 `sed -n 'A,Bp'` 只读那一段。
...
> ⚠️ 本节数字因测量输入被错误裁剪而作废，见 `docs/handoff/TICKET-gloss-coverage-remeasure.md`；结论以重测一节为准。
...
> 本节对应 `TICKET-gloss-blanks-and-cleanup.md`。本轮没有构建、OTA、内容包改动，也没有删除或暂存任何未跟踪内容目录。
...
|---:|---|---|---|
| 1 | n3_kurasu | 楽→舒适 | 正确 |
| 2 | n2_hantou | 朝→早上；た→田 | 全误 |
| 3 | n5_yasui | 服→衣服 | 正确 |
| 4 | n3_higai | 嵐→暴风雨 | 正确 |
| 5 | n3_hani | 私→我；超→超；え→画 | 后两条误 |
| 6 | n1_nyuushu | き→树 | 误 |
| 7 | n2_naika | 熱→发烧；あ→啊 | 后者误 |
| 8 | n1_gisei | 彼→他；な→名字；た→田 | 后两条误 |
| 9 | n1_kiwata | 品→物品；絹→丝绸；あ→啊 | 后者误 |
| 10 | n2_mendoukusai | 物→东西 | 正确 |
| 11 | n1_juugyouin | 時→时间；た→田 | 后者误 |
| 12 | n2_shitagaki | し→四；て→手 | 全误 |
| 13 | n1_hibi | 彼→他；た→田 | 后者误 |
| 14 | n5_migaku | 歯→牙齿 | 正确 |
| 15 | n1_koui_2 | 私×2；し；て；み；た | 私正确，其余误 |
| 16 | n2_touge | 峠；海 | 正确 |
| 17 | n1_oshiyoseru | 窓；高；た | 窓正确，其余误 |
| 18 | n4_tazuneru | 週；末；友；ち | 全误（词内碎片） |
| 19 | n2_hanabi | 空；光；い；た | 空正确，其余误 |
| 20 | n2_ukabu | 空；雲；ぶ | 前两条正确，ぶ误 |
| 21 | n3_haiyuu | 私；夢；ド；し；て；事 | 私/夢/事正确，其余误 |
| 22 | n1_gakusetsu | な；た | 全误 |
| 23 | n3_houbou | た | 误 |
| 24 | n5_ireru | て | 误 |
| 25 | n3_kuwashii | 君 | 正确 |
| 26 | n4_kisha | 昔 | 正确 |
| 27 | n3_kaburu | 億；円；た | 前两条正确，た误 |
| 28 | n3_hakaru | 熱；て；み | 熱正确，其余误 |
| 29 | n3_senkou | す | 误 |
| 30 | n1_kaku_2 | 角；左 | 正确 |

样本共 68 个单字命中，人工判为 38 个误命中，估计误命中率 **38/68 = 55.88%**。因此选择方案 **(a)**：只有当单字表面是当前 `EXAMPLE_TOKENS` 的独立 span 时才允许单字候选命中。它复用已有离线分词数据，规则可解释；没有选择 b（仍需边界启发式）或 c（会误伤独立单字词）。

### 机械修复

- `GRAMMAR` 增加稳定项 `られる`、`れる`、`だろう`、`だ`、`う`、`ょう`、`んだ`、`ん`；匹配仍按长度降序，故 `だろう` 先于 `だ`。
- 纯 ASCII/全角数字在语法表之外直接输出半角形式，例如 `２０２６→2026`。
- `buildWordFieldAlignment` 接受可选的当前 `EXAMPLE_TOKENS`，对单字候选和辞书形候选统一应用方案 (a)；旧调用不传该参数时保持兼容。覆盖脚本逐句传入对应 tokens。
- 新增测试覆盖稳定语法最长匹配、全角数字和单字独立 span 守卫；测试文件位于 `src/features/wordbank/__tests__/`。

### 修复后全量重测

输入仍是完整 **8,005** 条 `wordBank`、完整辞书形索引和 4,400 条有例句词条；脚本实际耗时 **92.09 秒**。拼回一致 **4,400/4,400（100.00%）**。

| 指标 | 修复前 | 修复后 | 变化 |
|---|---:|---:|---:|
| 非标点 gloss 覆盖 | 29,817/32,037 = 93.07% | 28,903/31,256 = 92.47% | -0.60pp |
| 全覆盖句 | 2,756/4,400 | 2,661/4,400 | -95 |
| 100% / 90–99% / 70–89% / <70% | 2,756 / 312 / 1,238 / 94 | 2,661 / 270 / 1,352 / 117 | — |
| 空白总数 | 2,233 | 2,365 | +132 |
| 贪心分词与 EXAMPLE_TOKENS 不同 | 2,416/4,400 | 2,085/4,400 | -331 |
| wordBank 单字 gloss | 6,390/18,763 = 34.06% | 4,681/17,088 = 27.39% | -6.67pp |
| 非标点单字 gloss | 未单列 | 14,469/28,903 = 50.06% | 新增观察 |

覆盖率下降是预期的准确性取舍：方案 (a) 拦截了大量词内单字误命中；不以覆盖率下降为理由回滚。

### 修复后空白成因

按工单指定的脚本判据，2,365 个空白分为：
...
“真·缺词”去重后 **604 个表面**，共 855 次，显著高于工单背景的约 50 个估计；以下是完整清单，按出现次数降序（同次数保持脚本输出顺序）：

```text
真·缺词 list (surface x count): 川×15 | 東京×15 | 週末×13 | 子ど×9 | 足×6 | 人々×5 | 天×5 | 田中×5 | 達×5 | 隊×5 | 駅前×5 | ％×4 | お前×4 | フランス語×4 | 中国×4 | 何時×4 | 私達×4 | 良×4 | 近づ×4 | 迷っ×4 | 野球×4 | ・・・×3 | 「×3 | 」×3 | 一杯×3 | 丸い×3 | 叔父×3 | 善悪×3 | 夕食×3 | 夜通し続けられ×3 | 大阪×3 | 師×3 | 時半×3 | 書名×3 | 浮×3 | 神戸×3 | 群衆×3 | 良い×3 | 英国×3 | 賞賛×3 | 超え×3 | 近年×3 | %×2 | Ａ×2 | オペレーション・トモダチ×2 | ご覧×2 | ソ連×2 | つトム×2 | め込×2 | ライ麦×2 | ロシア：×2 | 中秋×2 | 主×2 | 京都×2 | 仏教×2 | 佐藤×2 | 体重×2 | 何事×2 | 何人×2 | 何百×2 | 何色×2 | 使え×2 | 例文×2 | 侵×2 | 値上げ×2 | 健なり×2 | 出来×2 | 初め×2 | 制圧×2 | 受付×2 | 司令官×2 | 合い×2 | 合わ×2 | 名古屋×2 | 君子×2 | 哀悼×2 | 喫煙×2 | 圧倒×2 | 多額×2 | 奇跡×2 | 富士山×2 | 屋×2 | 山崎×2 | 島国×2 | 帰還×2 | 意×2 | 我×2 | 打ち寄せ×2 | 指先×2 | 排出×2 | 敗北×2 | 敵陣×2 | 昨晩×2 | 暑さ×2 | 暴徒×2 | 月見×2 | 月餅×2 | 有用×2 | 朝食×2 | 本心×2 | 校則×2 | 極秘×2 | 歯痛×2 | 無い×2 | 無色×2 | 特質×2 | 狂気×2 | 男女×2 | 界×2 | 発砲×2 | 眠れ×2 | 硫黄×2 | 視線×2 | 称され×2 | 稲穂×2 | 立派×2 | 紫色×2 | 羊×2 | 羽根×2 | 自ら強め×2 | 華×2 | 落石×2 | 蛇×2 | 表示×2 | 表裏×2 | 西岡×2 | 諸国×2 | 警×2 | 豪雨×2 | 軍部×2 | 込められ×2 | 郷×2 | 長さ×2 | 長島×2 | 長年×2 | 陣地×2 | 際限×2 | 難民×2 | 青空×2 | 青銅×2 | 風呂＆×2 | 飲酒×2 | 魔×2 | 黄銅×2 |  6000 ×1 | －１０℃×1 | ・・・女湯×1 | ，×1 | ? ×1 | 「ロミオ×1 | 」「×1 | ＋１０×1 | ＡＩ×1 | Australia×1 | Ｂ×1 | can×1 | ＣＤ×1 | IC×1 | Ｊ．Ｆ．ケネディ×1 | PKO×1 | Tatoeba×1 | Wi-Fi×1 | ウンコ漏れ×1 | えって具合×1 | お前ら×1 | お店×1 | キリスト教×1 | キロ痩せ×1 | キロ走り×1 | キロ足ら×1 | く似合×1 | く何×1 | く分×1 | く取っ×1 | く寝×1 | く有難×1 | く食べ×1 | けど使×1 | け会×1 | け早く×1 | ジュリエット」×1 | スパイダー ソリティア×1 | ソルト・レイク・シティー×1 | たらダメ×1 | たら渡っ×1 | つてイギリス×1 | つ年上×1 | ニ盈ツ×1 | ノ爆震ヲ聞キタリ×1 | ハ吾ガ×1 | ゆっくり行×1 | ゆっくり話×1 | ゆっくり食べ×1 | ら１１０×1 | らコンドーム×1 | ら始め×1 | ら戻し×1 | ら持っ×1 | ら歩き×1 | リチャード・ロ×1 | り方×1 | レッド・ホット×1 | れ怠け×1 | れ早×1 | ロシア人×1 | 一万×1 | 一億×1 | 一覧×1 | 万全×1 | 三十×1 | 三千×1 | 上半身×1 | 下ろし×1 | 下記×1 | 不断×1 | 不言×1 | 両国×1 | 両者×1 | 中国人×1 | 中級×1 | 丸薬×1 | 主な×1 | 乱用×1 | 五十×1 | 五千×1 | 井×1 | 人前×1 | 人参×1 | 人差し指×1 | 付×1 | 代わっ×1 | 仲良く×1 | 会え×1 | 会長×1 | 伝わせな×1 | 似×1 | 体調×1 | 何回食べ×1 | 何歳×1 | 使×1 | 保有×1 | 保障×1 | 傷口×1 | 傷痕×1 | 儘×1 | 光り輝×1 | 光陰×1 | 入国×1 | 入道雲×1 | 全力×1 | 全速×1 | 六十×1 | 其×1 | 円高×1 | 最上×1 | 最新×1 | 最良×1 | 冷×1 | 冷夏×1 | 凌ぎ×1 | 凍結×1 | 凡俗×1 | 切っ×1 | 到頭×1 | 制す×1 | 前面×1 | 動画×1 | 勝て×1 | 勿れ×1 | 十二支×1 | 十八×1 | 半数×1 | 半球×1 | 南口×1 | 南洋フーズ×1 | 博×1 | 占い師×1 | 卵黄×1 | 原宿×1 | 参列×1 | 反面×1 | 史×1 | 各×1 | 各色×1 | 各駅×1 | 合×1 | 合っ×1 | 名古屋市×1 | 名案×1 | 吹っ飛×1 | 吾×1 | 告発×1 | 善意×1 | 喋る×1 | 喘息×1 | 四辺×1 | 団×1 | 団欒×1 | 国旗×1 | 地面×1 | 埋葬×1 | 執行×1 | 堅×1 | 報われ×1 | 報復×1 | 塗り×1 | 塗り直×1 | 墜×1 | 士×1 | 売り×1 | 売れ×1 | 変え×1 | 夏祭り×1 | 夜通×1 | 大っ×1 | 大型×1 | 大声×1 | 大昔×1 | 天引き×1 | 太郎×1 | 失せ×1 | 失礼×1 | 奈良×1 | 奴隷×1 | 奴隷制×1 | 如く×1 | 如し×1 | 姫×1 | 婆×1 | 媒介×1 | 子猫×1 | 安楽×1 | 官×1 | 定刻×1 | 客達×1 | 宣告×1 | 宣戦×1 | 容疑×1 | 寄×1 | 富士×1 | 寛恕×1 | 寝×1 | 寝室×1 | 小脳×1 | 少量×1 | 屈託×1 | 届×1 | 履い×1 | 履き×1 | 山中×1 | 山分け×1 | 山田×1 | 川添×1 | 左側×1 | 左足×1 | 己×1 | 市長×1 | 帝国×1 | 干物×1 | 年越し×1 | 度々×1 | 度目×1 | 延び×1 | 引っ越×1 | 引出×1 | 強固×1 | 彗星×1 | 後１０×1 | 後世×1 | 後半減×1 | 心付け×1 | 心頭×1 | 必須×1 | 忌まわしいハエ×1 | 忍耐力×1 | 志賀×1 | 忠言×1 | 念願×1 | 急病×1 | 性交×1 | 恐竜×1 | 恒星×1 | 患い×1 | 惚れ×1 | 惹×1 | 愛知県×1 | 愛読×1 | 感×1 | 感嘆×1 | 感嘆符×1 | 感服×1 | 懸命×1 | 懸命働×1 | 成ら×1 | 戦慄×1 | 戻せ×1 | 手ぶ×1 | 打ち×1 | 打ち勝っ×1 | 打倒×1 | 指名×1 | 振れ×1 | 振込×1 | 探偵×1 | 控訴×1 | 搭乗×1 | 放火×1 | 放題×1 | 救出×1 | 数千ドル×1 | 文頭×1 | 断り×1 | 新作×1 | 新居×1 | 新潟×1 | 新社屋×1 | 新車×1 | 日差し×1 | 早め×1 | 昔々×1 | 映×1 | 暗澹×1 | 書×1 | 有×1 | 朋有り×1 | 朝型×1 | 朝鮮×1 | 木々×1 | 木星×1 | 木造×1 | 札幌×1 | 村人×1 | 来れ×1 | 東京駅×1 | 林立×1 | 栄枯盛衰×1 | 棘×1 | 概して×1 | 標的×1 | 横浜×1 | 欠×1 | 欲せざる×1 | 歓心×1 | 正座×1 | 正月×1 | 死すべ×1 | 死者×1 | 気晴らし×1 | 水漏れ×1 | 水着×1 | 氷山×1 | 汎神論×1 | 泳げる×1 | 活×1 | 流暢×1 | 流水×1 | 浅間×1 | 海王星×1 | 海賊×1 | 海辺×1 | 浸水×1 | 深さ×1 | 済×1 | 満塁×1 | 満腹×1 | 満開×1 | 滅び×1 | 潔白×1 | 潜伏×1 | 災い×1 | 為め×1 | 無罪×1 | 照れな×1 | 熊×1 | 狐×1 | 狩人×1 | 猛×1 | 猟師×1 | 王位×1 | 球体×1 | 瓜×1 | 産卵×1 | 疾患×1 | 痒い×1 | 登れ×1 | 白色×1 | 百万ドル×1 | 的中×1 | 的人権×1 | 皇帝×1 | 盲目×1 | 真×1 | 真夏×1 | 破滅×1 | 神明×1 | 視力×1 | 禁固１×1 | 福岡×1 | 積×1 | 空前×1 | 窓側×1 | 竜×1 | 精通×1 | 純子×1 | 絹糸×1 | 継い×1 | 継ぎ×1 | 緑色×1 | 署×1 | 罰則×1 | 罰金×1 | 翌朝×1 | 聡明×1 | 聴×1 | 聴い×1 | 聴きな×1 | 肥え×1 | 肥沃×1 | 育む×1 | 胃痛×1 | 脳死×1 | 自助×1 | 自室×1 | 自慰×1 | 致命的×1 | 舞踏会×1 | 良く×1 | 良くなっ×1 | 良薬×1 | 苗字×1 | 若者×1 | 苦なり×1 | 苺×1 | 蒲団×1 | 虚偽×1 | 虫食わ×1 | 蜘蛛×1 | 血まみれ×1 | 行方×1 | 被告×1 | 見×1 | 見習いな×1 | 親和性×1 | 試練×1 | 話せ×1 | 話せる×1 | 話者×1 | 読め×1 | 販促×1 | 貯め×1 | 資質×1 | 赤々×1 | 赤色×1 | 足買っ×1 | 路線×1 | 込×1 | 返信×1 | 迷ったら×1 | 通れ×1 | 通例×1 | 連×1 | 進捗×1 | 遅×1 | 運行×1 | 過信×1 | 遺伝子×1 | 遺言書×1 | 重さ×1 | 量る×1 | 釣魚×1 | 錠×1 | 鍛冶屋×1 | 長居×1 | 長旅×1 | 長生き×1 | 長野×1 | 開演×1 | 間あり×1 | 降ろ×1 | 限り×1 | 階上×1 | 雄弁×1 | 離陸×1 | 電器×1 | 靴屋×1 | 靴紐×1 | 韓国×1 | 音痴×1 | 頑張っ×1 | 領有権×1 | 食×1 | 餃子×1 | 餓死×1 | 餓死寸前×1 | 館内×1 | 駐米×1 | 魔女×1 | 鯉×1 | 鳴っ×1 | 鳴り×1 | 鶏×1 | 鶏卵×1 | 黒海×1 | 黒色×1
```

上面为报告摘要而非外部词源判定；这批只列清单，不用 JMdict 或 LLM 补中文。尤其其中不少表面仍可能是分词/语料边界问题，不能直接视为应落库词条。

### 仓库结构清理

- 新增根目录 `.gitignore`，排除 `YanApp_backup_0501/`、`resources/`、`YanApp/调研/` 及 `yan-content/` 下列出的旧文件；没有删除，也没有 `git add` 这些未跟踪物。
- 新增 [`docs/handoff/UNTRACKED-INVENTORY.md`](docs/handoff/UNTRACKED-INVENTORY.md)，逐项记录大小、时间、用途和保留/删除建议。`content.json` 与 `content.v1.json` 已单独标红为权威 `content.v2.json` 旁的高风险旧包，建议由负责人确认后再删除。
- 指定 grep 检查无输出：没有 `*.mjs/*.js/*.sh/*.py` 脚本读取 `content.v1.json` 或 `yan-content/content.json`。

### 本轮门禁

最终 raw `git status --short`、`npm test && npm run typecheck && npm run audit` 输出如下；audit 的既有文档引用问题如实保留，未借本工单顺手修复。没有构建或发 OTA。
...
PASS invariant wordBank.total=8005; _meta.note=8005
PASS metric publication.learning=1187 (not asserted)
INFO doc-refs scanned 919 references (399 unique)
WARN doc-refs docs/AUDIT-source-trust-2026-08-22.md:16: missing 调研/…/red调研重新规划_编号修正版.md
WARN doc-refs docs/AUDIT-source-trust-2026-08-22.md:286: missing red调研重新规划_编号修正版.md
WARN doc-refs docs/ROADMAP-content-trust-structure-ui.md:804: missing src/content/publication.ts
WARN doc-refs docs/ROADMAP-content-trust-structure-ui.md:803: missing src/content/schema.ts
...
> 本节是 `docs/handoff/TICKET-gloss-coverage-remeasure.md` 的重测结果，覆盖上一节已作废数字。上一轮脚本的 `scopedInputs()` 已删除；本次每条句子都直接传入完整 `content.wordBank`（8,005 条）和完整 `dictionaryFormsFrom(EXAMPLE_TOKENS)`。没有改 `wordFieldAlignment.js`、App/UI 或内容包，没有构建、没有发 OTA。实际运行耗时：**143.54 秒（real）**。

### 重测结果

- 有例句词条：**4,400**。
- 可用率（拼回原句）：**4,400 / 4,400（100.00%）**。
- Gloss 覆盖（排除标点）：**29,817 / 32,037（93.07%）**。
- 全覆盖句：**2,756 / 4,400**。
- 覆盖分布：**100% 2,756**；**90–99% 312**；**70–89% 1,238**；**<70% 94**。
- 空白成因（总空白 token **2,233**）：**活用碎片 140（6.27%）**；**表记差异 446（19.97%）**；**不在词库 1,647（73.76%）**。
- 贪心分词与 `EXAMPLE_TOKENS` 不同：**2,416 / 4,400**；可比较项完整，无缺失。
- 拼回一致性再次确认：**4,400 / 4,400**。

这次 `blankCause()` 不再直接把 runtime 的 `blankKind` 当成三类结果：脚本将空白 token 的表面、以及其 `EXAMPLE_TOKENS` 重叠 token 的辞书形与词库 `word/reading` 集合比对；命中辞书形计入活用碎片，命中表面计入表记差异，二者都不命中才计入不在词库。

### 与上一轮数字的对照表

| 指标 | 上一轮（作废） | 本次全量重测 | 变化 |
|---|---:|---:|---:|
| 拼回一致 | 4,400/4,400（100.00%） | 4,400/4,400（100.00%） | 不变 |
| Gloss 覆盖 | 29,521/33,566（87.95%） | 29,817/32,037（93.07%） | **+5.12 个百分点**；分子 +296、分母 -1,529 |
| 全覆盖句 | 1,622 | 2,756 | **+1,134** |
| 100% | 1,622 | 2,756 | +1,134 |
| 90–99% | 378 | 312 | -66 |
| 70–89% | 2,200 | 1,238 | -962 |
| <70% | 200 | 94 | -106 |
| 空白 token 总数 | 4,058 | 2,233 | -1,825 |
| 空白：活用碎片 | 2,627（64.74%） | 140（6.27%） | 计数 -2,487；分类口径已修正 |
| 空白：表记差异 | 1,431（35.26%） | 446（19.97%） | 计数 -985；分类口径已修正 |
| 空白：不在词库 | 0（0.00%） | 1,647（73.76%） | **不再是死代码结果** |
| 贪心 vs `EXAMPLE_TOKENS` | 3,074 | 2,416 | **-658** |

上一轮最低覆盖样本中的 `散れ！`、`黙れ！` 不再是最低样本：全量输入下分别为 `散れ→凋谢 | ！→！`、`黙れ→沉默 | ！→！`，覆盖均为 **100%**。`見せます。` 也走到辞书形命中：`見せ→给……看 | ます→（礼貌） | 。→。`。

### 稳定随机样本（15 条）

固定 hash 顺序保持与上一轮相同；`∅` 表示 gloss 留空。

1. `n3_kurasu`｜彼女は安楽に暮らしている。  
   `彼女→她 | は→（主题） | 安→∅ | 楽→舒适 | に→（向/于） | 暮らし→生活 | てい→样子 | る→∅ | 。→。`；覆盖 75.00%；空白：安（不在词库）、る（表记差异）。
2. `n2_hantou`｜朝鮮半島を訪れたことがありますか。  
   `朝→早上 | 鮮→∅ | 半島→半岛 | を→（宾语） | 訪れ→拜访 | た→田 | こと→日本筝 | が→（主语） | あり→有 | ます→（礼貌） | か→（疑问） | 。→。`；覆盖 90.91%；空白：鮮（不在词库）。
3. `n5_paateii`｜パーティーに行きます。  
   `パーティー→派对 | に→（向/于） | 行き→去程 | ます→（礼貌） | 。→。`；覆盖 100.00%；空白：无。
4. `n1_muchi`｜無知は幸福。  
   `無知→无知 | は→（主题） | 幸福→幸福 | 。→。`；覆盖 100.00%；空白：无。
5. `n5_yasui`｜安い服を探しています。  
   `安い→便宜的 | 服→衣服 | を→（宾语） | 探し→找（商品） | てい→样子 | ます→（礼貌） | 。→。`；覆盖 100.00%；空白：无。
6. `n3_higai`｜屋根は嵐の被害を受けました。  
   `屋根→屋顶 | は→（主题） | 嵐→暴风雨 | の→（的） | 被害→受害 | を→（宾语） | 受け→接受 | ました→（过去） | 。→。`；覆盖 100.00%；空白：无。
7. `n4_atsumeru`｜古い切手を集めています。  
   `古い→旧的 | 切手→邮票 | を→（宾语） | 集め→收集 | てい→样子 | ます→（礼貌） | 。→。`；覆盖 100.00%；空白：无。
8. `n5_hagaki`｜葉書を送る。  
   `葉書→明信片 | を→（宾语） | 送る→发送 | 。→。`；覆盖 100.00%；空白：无。
9. `n3_hani`｜これは私の想像の範囲を超えている。  
   `これ→这个 | は→（主题） | 私→我（郑重说法） | の→（的） | 想像→想象 | の→（的） | 範囲→范围 | を→（宾语） | 超→超 | え→画 | てい→样子 | る→∅ | 。→。`；覆盖 91.67%；空白：る（表记差异）。
10. `n1_nyuushu`｜登録用紙は無料で入手できます。  
    `登録→登记 | 用紙→表格用纸 | は→（主题） | 無料→免费 | で→（在/用） | 入手→得到 | で→（在/用） | き→树 | ます→（礼貌） | 。→。`；覆盖 100.00%；空白：无。
11. `n2_naika`｜熱があるなら内科ですね。  
    `熱→发烧 | が→（主语） | あ→啊 | る→∅ | なら→变成 | 内科→内科 | です→（是） | ね→（确认） | 。→。`；覆盖 87.50%；空白：る（表记差异）。
12. `n1_gisei`｜彼は成功するのに非常な犠牲をはらった。  
    `彼→他 | は→（主题） | 成功→成功 | する→做 | の→（的） | に→（向/于） | 非常→紧急 | な→名字 | 犠牲→牺牲 | を→（宾语） | は→（主题） | らっ→∅ | た→田 | 。→。`；覆盖 92.31%；空白：らっ（活用碎片）。
13. `n1_kiwata`｜彼らの主たる輸出品は織物であり、特に絹と木綿である。  
    `彼ら→他们 | の→（的） | 主→∅ | たる→足够 | 輸出→出口 | 品→物品 | は→（主题） | 織物→纺织品 | で→（在/用） | あり→有 | 、→、 | 特に→特别 | 絹→丝绸 | と→（和/与） | 木綿→棉 | で→（在/用） | あ→啊 | る→∅ | 。→。`；覆盖 88.24%；空白：主（不在词库）、る（表记差异）。
14. `n2_mendoukusai`｜洗い物が面倒くさい。  
    `洗い→洗 | 物→东西（具体物品） | が→（主语） | 面倒→麻烦 | くさい→臭 | 。→。`；覆盖 100.00%；空白：无。
15. `n1_juugyouin`｜６時を過ぎると従業員は帰り始めた。  
    `６→∅ | 時→时间 | を→（宾语） | 過ぎる→超过 | と→（和/与） | 従業員→员工 | は→（主题） | 帰り→回来 | 始め→开始 | た→田 | 。→。`；覆盖 90.00%；空白：６（不在词库）。

### 最低覆盖样本（5 条）

1. `n3_kane`｜華金だ！：`華→∅ | 金→钱 | だ→∅ | ！→！`；覆盖 **33.33%**；空白：華（不在词库）、だ（不在词库）。
2. `n3_kin`｜華金だ！：`華→∅ | 金→钱 | だ→∅ | ！→！`；覆盖 **33.33%**；空白：華（不在词库）、だ（不在词库）。
3. `n3_kai_2`｜９回２死満塁。：`９→∅ | 回→次 | ２→∅ | 死→死亡 | 満塁→∅ | 。→。`；覆盖 **40.00%**；空白：９（不在词库）、２（不在词库）、満塁（不在词库）。
4. `n1_atarimae`｜当たり前だ。：`当たり前→理所当然 | だ→∅ | 。→。`；覆盖 **50.00%**；空白：だ（不在词库）。
5. `n1_bakudan`｜吾等、爆弾ノ爆震ヲ聞キタリ。：`吾→∅ | 等→等等 | 、→、 | 爆弾→炸弹 | ノ爆震ヲ聞キタリ→∅ | 。→。`；覆盖 **50.00%**；空白：吾（不在词库）、ノ爆震ヲ聞キタリ（不在词库）。

### 单字 gloss 命中率与 20 条样本

- 所有来源的单字 gloss token：**20,352 / 34,634（58.76%）**。
- 排除标点后的单字 gloss token：**15,535 / 29,817（52.10%）**。
- 更直接对应误命中风险的词库单字 gloss：**6,390 / 18,763 个有 gloss 的词库 token（34.06%）**。

以下 20 条只列词库来源、长度为 1 且有 gloss 的命中；完整 token 行同时保留，便于看出语境。它们包括真实单字词，也包括需要警惕的短片段误命中（如 `た→田`、`え→画`、`き→树`）。

1. `n3_kurasu`｜彼女は安楽に暮らしている。命中：`楽→舒适`。完整：`彼女→她 | は→（主题） | 安→∅ | 楽→舒适 | に→（向/于） | 暮らし→生活 | てい→样子 | る→∅ | 。→。`
2. `n2_hantou`｜朝鮮半島を訪れたことがありますか。命中：`朝→早上 | た→田`。完整：`朝→早上 | 鮮→∅ | 半島→半岛 | を→（宾语） | 訪れ→拜访 | た→田 | こと→日本筝 | が→（主语） | あり→有 | ます→（礼貌） | か→（疑问） | 。→。`
3. `n5_yasui`｜安い服を探しています。命中：`服→衣服`。完整：`安い→便宜的 | 服→衣服 | を→（宾语） | 探し→找（商品） | てい→样子 | ます→（礼貌） | 。→。`
4. `n3_higai`｜屋根は嵐の被害を受けました。命中：`嵐→暴风雨`。完整：`屋根→屋顶 | は→（主题） | 嵐→暴风雨 | の→（的） | 被害→受害 | を→（宾语） | 受け→接受 | ました→（过去） | 。→。`
5. `n3_hani`｜これは私の想像の範囲を超えている。命中：`私→我（郑重说法） | 超→超 | え→画`。完整：`これ→这个 | は→（主题） | 私→我（郑重说法） | の→（的） | 想像→想象 | の→（的） | 範囲→范围 | を→（宾语） | 超→超 | え→画 | てい→样子 | る→∅ | 。→。`
6. `n1_nyuushu`｜登録用紙は無料で入手できます。命中：`き→树`。完整：`登録→登记 | 用紙→表格用纸 | は→（主题） | 無料→免费 | で→（在/用） | 入手→得到 | で→（在/用） | き→树 | ます→（礼貌） | 。→。`
7. `n2_naika`｜熱があるなら内科ですね。命中：`熱→发烧 | あ→啊`。完整：`熱→发烧 | が→（主语） | あ→啊 | る→∅ | なら→变成 | 内科→内科 | です→（是） | ね→（确认） | 。→。`
8. `n1_gisei`｜彼は成功するのに非常な犠牲をはらった。命中：`彼→他 | な→名字 | た→田`。完整：`彼→他 | は→（主题） | 成功→成功 | する→做 | の→（的） | に→（向/于） | 非常→紧急 | な→名字 | 犠牲→牺牲 | を→（宾语） | は→（主题） | らっ→∅ | た→田 | 。→。`
9. `n1_kiwata`｜彼らの主たる輸出品は織物であり、特に絹と木綿である。命中：`品→物品 | 絹→丝绸 | あ→啊`。完整：`彼ら→他们 | の→（的） | 主→∅ | たる→足够 | 輸出→出口 | 品→物品 | は→（主题） | 織物→纺织品 | で→（在/用） | あり→有 | 、→、 | 特に→特别 | 絹→丝绸 | と→（和/与） | 木綿→棉 | で→（在/用） | あ→啊 | る→∅ | 。→。`
10. `n2_mendoukusai`｜洗い物が面倒くさい。命中：`物→东西（具体物品）`。完整：`洗い→洗 | 物→东西（具体物品） | が→（主语） | 面倒→麻烦 | くさい→臭 | 。→。`
11. `n1_juugyouin`｜６時を過ぎると従業員は帰り始めた。命中：`時→时间 | た→田`。完整：`６→∅ | 時→时间 | を→（宾语） | 過ぎる→超过 | と→（和/与） | 従業員→员工 | は→（主题） | 帰り→回来 | 始め→开始 | た→田 | 。→。`
12. `n2_shitagaki`｜まず下書きをしてから清書する。命中：`し→四 | て→手`。完整：`まず→首先 | 下書き→草稿 | を→（宾语） | し→四 | て→手 | から→（从） | 清書→誊写 | する→做 | 。→。`
13. `n1_hibi`｜彼は忙しい日々を送った。命中：`彼→他 | た→田`。完整：`彼→他 | は→（主题） | 忙しい→忙的 | 日々→每天 | を→（宾语） | 送っ→发送 | た→田 | 。→。`
14. `n5_migaku`｜歯を磨きます。命中：`歯→牙齿`。完整：`歯→牙齿 | を→（宾语） | 磨き→刷（牙） | ます→（礼貌） | 。→。`
15. `n1_koui_2`｜私は私の行為を犯罪としてみた。命中：`私→我（郑重说法） | 私→我（郑重说法） | し→四 | て→手 | み→身体 | た→田`。完整：`私→我（郑重说法） | は→（主题） | 私→我（郑重说法） | の→（的） | 行為→行为 | を→（宾语） | 犯罪→犯罪 | と→（和/与） | し→四 | て→手 | み→身体 | た→田 | 。→。`
16. `n2_touge`｜峠を越えると海が見える。命中：`峠→山顶 | 海→海`。完整：`峠→山顶 | を→（宾语） | 越える→越过 | と→（和/与） | 海→海 | が→（主语） | 見える→看得见 | 。→。`
17. `n1_oshiyoseru`｜洪水は窓の高さまで押し寄せた。命中：`窓→窗户 | 高→数量 | た→田`。完整：`洪水→洪水 | は→（主题） | 窓→窗户 | の→（的） | 高→数量 | さま→先生 | で→（在/用） | 押し寄せ→涌来 | た→田 | 。→。`
18. `n4_tazuneru`｜週末に友だちを訪ねます。命中：`週→周 | 末→末端 | 友→朋友 | ち→血`。完整：`週→周 | 末→末端 | に→（向/于） | 友→朋友 | だ→∅ | ち→血 | を→（宾语） | 訪ね→拜访 | ます→（礼貌） | 。→。`
19. `n2_hanabi`｜空は花火で光り輝いていた。命中：`空→天空 | 光→光 | い→胃 | た→田`。完整：`空→天空 | は→（主题） | 花火→烟花 | で→（在/用） | 光→光 | り輝→∅ | い→胃 | てい→样子 | た→田 | 。→。`
20. `n2_ukabu`｜空に浮かぶ雲が美しい。命中：`空→天空 | ぶ→不 | 雲→云`。完整：`空→天空 | に→（向/于） | 浮→∅ | か→（疑问） | ぶ→不 | 雲→云 | が→（主语） | 美しい→美丽 | 。→。`

这组产品风险比 93.07% 覆盖率更重要：词库来源单字 gloss 占所有词库 gloss token 的 **34.06%**，其中 `た→田`、`え→画`、`き→树`、`め→眼睛` 等不是留空，而是会被用户当成正确解释的误命中。本轮只记录，不修正。

### 本轮实际改动与门禁

- 只改 `scripts/gloss-coverage.mjs` 的测量口径：删掉 `scopedInputs()`，改为全量输入；脚本内新增三类空白成因判定、单字统计与样本输出。
- 追加本节并更新 `ACTIVE.md`；`staging/deep-card-audit.md` 未改，深卡结论沿用上一轮。
...
ℹ skipped 0
ℹ todo 0
ℹ duration_ms 14266.581362

> yanapp@1.0.0 typecheck
> tsc --noEmit
~~~

随后执行 npm run audit 的原始输出：
~~~
> yanapp@1.0.0 audit
> node scripts/audit.mjs
...
PASS invariant wordBank.total=8005; _meta.note=8005
PASS metric publication.learning=1187 (not asserted)
INFO doc-refs scanned 1055 references (439 unique)
WARN doc-refs docs/AUDIT-source-trust-2026-08-22.md:16: missing 调研/…/red调研重新规划_编号修正版.md
WARN doc-refs docs/AUDIT-source-trust-2026-08-22.md:286: missing red调研重新规划_编号修正版.md
WARN doc-refs docs/handoff/CC-REPORT.md:3881: missing src/content/publication.ts
WARN doc-refs docs/handoff/CC-REPORT.md:3882: missing src/content/schema.ts
WARN doc-refs docs/handoff/CC-REPORT.md:3883: missing src/content/contentValidation.ts
WARN doc-refs docs/handoff/CC-REPORT.md:3884: missing staging/duplicate-seq-plan.md
WARN doc-refs docs/handoff/CC-REPORT.md:3885: missing staging/duplicate-seq-groups.json
...
WARN user-claims App.js:3183: review editorial claim "旅行最高频框架"
WARN user-claims src/features/kana/KanaScreen.js:1954: review editorial claim "旅行高频"
PASS content-pack-sync sha256 88acbd38408a474c09b4499096b623d0d03a13035eff6ba70c3fe8edbb1e9f7e
PASS content-pack-sync authority content.v2.json has no uncommitted change
PASS content-pack-sync version/content comparison
PASS invariant kanji_anchor.total=563
PASS invariant wordBank.total=8005; _meta.note=8005
PASS metric publication.learning=1187 (not asserted)
INFO doc-refs scanned 1135 references (473 unique)
WARN doc-refs docs/AUDIT-source-trust-2026-08-22.md:16: missing 调研/…/red调研重新规划_编号修正版.md
WARN doc-refs docs/AUDIT-source-trust-2026-08-22.md:286: missing red调研重新规划_编号修正版.md
WARN doc-refs docs/handoff/CC-REPORT.md:4192: missing src/content/publication.ts
WARN doc-refs docs/handoff/CC-REPORT.md:4193: missing src/content/schema.ts
WARN doc-refs docs/handoff/CC-REPORT.md:4194: missing src/content/contentValidation.ts
WARN doc-refs docs/handoff/CC-REPORT.md:4195: missing staging/duplicate-seq-plan.md
WARN doc-refs docs/handoff/CC-REPORT.md:4196: missing staging/duplicate-seq-groups.json
WARN doc-refs docs/handoff/ACTIVE.md:40: missing TICKET-wordfield-zh-38.md
WARN doc-refs docs/handoff/ACTIVE.md:41: missing TICKET-wordfield-lv-67.md
WARN doc-refs docs/handoff/TICKET-correction-entry-minimal.md:62: missing scripts/corrections-export.mjs
PASS doc-refs 所有引用都已入库（10 条指向不存在的路径，见 WARN）
PASS workspace-clean docs markdown tracked
--- audit summary ---
FAIL: 0
WARN: 17
...
1. 本轮没有重新选择句子；343 条及其五状态总数不变。新增的中文信号从 8 条变为 9 条，原因只有一个：把工单点名的 n5_fun 语气错译从 DATA 背后显式标成 FIX_ZH；不是重新发现了一批隐藏数据。
2. 没有说不清来源的数字。唯一的决策数字 6 / 40 是人工判读，不是脚本分类结果；判读对象、判据和完整样本均在本节。
3. “中文错译”三类的归类是人工决定，不是自动测量；多标签数量由脚本按固定标签优先级生成。LAND 仍只表示没有命中固定机械信号，不表示中文已经得到母语者确认。

### 决策指标

本轮唯一交付物是 LAND 301 条中按 anchor_id 排序取前 40 条的人工判读。按“意思错 / 意思对但不像人话 / 意思对但语体不对”任一类计为中文有问题：

- 意思错：1 条
- 意思对但不像人话：4 条
- 意思对、也通顺，但和日语语体对不上：1 条
- 合计：6 / 40 = 15%

这是人工判读数：对象是下面完整贴出的 40 条；判据是逐句比较日语的主体、动作、方向、时态和语气，再检查中文自然度与语体。它不落入工单设定的“低于 5%”或“20% 以上”两端阈值，因此本轮不能据此宣布 LAND 可直接落，也不能据此宣布全部中文必须先返工。

### 多标签改造

每行现在带 review_labels 数组；review_status 仍是最高优先级动作。标签优先级保持 DATA > SPOKEN > SWAP > FIX_ZH > LAND，因此状态不会被改写，但其他标签不会被吞掉。

重排后有 2 条带至少两个标签：
- n5_ane：SWAP + FIX_ZH
- n5_fun：DATA + FIX_ZH

n5_fun 的 DATA 不再遮住中文信号：もう５分待とう。是意向形“再等五分钟吧”，当前“我还要等五分钟”漏掉了提议语气。n5_ane 的中文信号也保留在 FIX_ZH 标签中，即使最高动作仍是 SWAP。

复算：
~~~
node scripts/wordfield-shortlist.mjs | grep -E '^(statuses|multi-label rows|Chinese signals):'
~~~

### 40 条人工样本

样本规则是：脚本输出按 anchor_id 排序后，筛 review_status=LAND，再取前 40 条。没有随机抽样，没有使用模型改写任何文本。表中的 Tatoeba ID 仅用于定位原句和对齐译文。

复算：
~~~
node - <<'NODE'
const r=require('./staging/wordfield-shortlist-343.json');
for (const x of r.filter(x=>x.review_status==='LAND').slice(0,40))
  console.log(x.anchor_id,x.jp,x.zh,x.tatoeba);
NODE
~~~

| # | anchor | 日语原句 | 当前中文 | 判读 | Tatoeba 日 / 中 ID |
|---:|---|---|---|---|---|
| 1 | n5_abiru | 姉は毎朝シャワーを浴びます。 | 我姐姐每天都会洗澡。 | 对 | 163535 / 432945 |
| 2 | n5_ageru | 彼女は頭を上げた。 | 她抬起了头。 | 对 | 87708 / 11254309 |
| 3 | n5_aka | 私の大好きな色は赤です。 | 我最喜欢红色。 | 对 | 163100 / 9955413 |
| 4 | n5_akai | 赤い屋根の家を見てごらん。 | 看看那间红色屋顶的房子。 | 对 | 1151723 / 348624 |
| 5 | n5_akarui | 私は明るい色が好きです。 | 我喜欢明亮的颜色。 | 对 | 1820284 / 4455614 |
| 6 | n5_akeru | この箱の開け方分かる？ | 你知道如何打开这个箱子吗？ | 对 | 3453243 / 872376 |
| 7 | n5_aki | 私は秋より春が好きだ。 | 我喜欢春天胜过秋天。 | 对 | 156041 / 848694 |
| 8 | n5_aku | 靴下に穴が開いているよ。 | 你的袜子破了一个洞呀。 | 对 | 179233 / 8593083 |
| 9 | n5_ame | 雨は一週間降り続いた。 | 雨下了一周。 | 对 | 189550 / 333989 |
| 10 | n5_ani | 私は兄が八人います。 | 我有八个哥哥。 | 对 | 3418139 / 8806670 |
| 11 | n5_ao | 彼女の目の色は青です。 | 她眼睛是蓝色的。 | 意思对但不像人话：缺少“的”，中文不完整 | 93887 / 9961662 |
| 12 | n5_aoi | 私の目は青いです。 | 我的眼睛是蓝色的。 | 对 | 422364 / 794634 |
| 13 | n5_arau | トイレから出たら手を洗いましょう！ | 便后请洗手。 | 意思对、语体不对：日语是一起提议，中文像公示语 | 347251 / 347252 |
| 14 | n5_aru_2 | 彼は学生に人気が有る。 | 他受到学生的欢迎。 | 对 | 108834 / 848691 |
| 15 | n5_aruku | 少し歩くと駅に出ます。 | 稍微走两步就能出车站。 | 意思错：“駅に出ます”是走到/到达车站，不是离开车站 | 146778 / 13942584 |
| 16 | n5_asa | 彼女は朝七時に起きた。 | 她早上七点起床的。 | 对 | 1167602 / 751862 |
| 17 | n5_ashi | 靴がきつくて足が痛い。 | 这些鞋子太紧了，伤脚。 | 对 | 179262 / 1783815 |
| 18 | n5_ashita | 明日は一日家にいます。 | 明天我一整天都在家。 | 对 | 4804659 / 5100101 |
| 19 | n5_asobu | 私は庭で遊びます。 | 我在花园里玩。 | 对 | 2786084 / 2785224 |
| 20 | n5_atarashii | 新しい自転車を買いたい。 | 我想买一辆新的自行车。 | 对 | 145385 / 9453438 |
| 21 | n5_atatakai | 三月にはもっと暖かくなるだろう。 | 到了三月会变的更温暖吧 | 意思对但不像人话：应为“变得”，中文用字不规范 | 169507 / 5849914 |
| 22 | n5_ato | 後で取りに来ます。 | 我晚点再来拿。 | 对 | 174339 / 8789683 |
| 23 | n5_atsui | 今日は何て暑いんだ。 | 今天多么地热啊! | 意思对但不像人话：“多么地热”生硬 | 171694 / 801536 |
| 24 | n5_atsui_3 | あの厚い本を読んでたの？ | 你有没有看那本厚厚的书？ | 对 | 5070660 / 361865 |
| 25 | n5_au | 来週の今日お会いします。 | 我下星期的今天会和你见面。 | 意思对但不像人话：“下星期的今天”直译腔 | 78649 / 891265 |
| 26 | n5_ban | 彼女は一晩中泣き通した。 | 她一整晚都在哭。 | 对 | 91111 / 333193 |
| 27 | n5_ban_2 | 今度は私が運転する番よ。 | 下次该轮到我开车了。 | 对；“今度”脱离语境可指这次或下次，不武断判错 | 172097 / 8877852 |
| 28 | n5_benkyou | 私は英語を勉強します。 | 我学英语。 | 对 | 158294 / 10448719 |
| 29 | n5_boushi | 帽子を脱いで下さい。 | 请您摘下您的帽子。 | 对 | 82593 / 343263 |
| 30 | n5_bunshou | 先生、この文章は正しいですか？ | 老师，这句话正确吗？ | 对 | 11243008 / 13920702 |
| 31 | n5_butaniku | 豚肉は私には合わない。 | 猪肉不适合我。 | 对 | 123261 / 8789278 |
| 32 | n5_byouin | 近くに病院がある。 | 附近有一家医院。 | 对 | 179875 / 9961491 |
| 33 | n5_byouki | 彼は病気で学校を休んだ。 | 他病了，所以没有上学。 | 对 | 1050411 / 563669 |
| 34 | n5_chairo | その猫は茶色です。 | 那只猫是棕色的。 | 对 | 982253 / 13605729 |
| 35 | n5_chichi | 父は車で仕事に行きます。 | 我父亲开车去上班。 | 对 | 84458 / 834890 |
| 36 | n5_chigau | 君は違う鍵を取った。 | 你拿错钥匙了。 | 对 | 1019501 / 1776658 |
| 37 | n5_chiisai | もう少し小さい声で話してください。 | 请再小声一点说话。 | 对 | 8586330 / 10275160 |
| 38 | n5_chikaku | 近くで犬が吠えている。 | 附近有狗在叫。 | 对 | 2860740 / 2843700 |
| 39 | n5_chikatetsu | 私は地下鉄で学校に行く。 | 我坐地铁上学。 | 对 | 155130 / 9524569 |
| 40 | n5_chizu | 地図は壁に貼ってある。 | 地图挂在墙上。 | 对 | 991942 / 8869384 |

### 来源风险

中文来自 Tatoeba 的 zh 对齐句，是用户贡献内容，质量不齐；本次 6 / 40 的问题应归因于这批来源译文的质量差异，不能描述成 shortlist 筛选规则本身制造了错译。

### 本轮边界与提交

本轮只改了 scripts/wordfield-shortlist.mjs、staging/wordfield-shortlist-343.json、ACTIVE.md 和本节报告；没有改任何日文或中文，没有改两个内容包，没有实现 register，没有构建或发 OTA。想顺手修正这 6 条中文但忍住没改，下一步需由负责人决定是否开启中文返工工单。
