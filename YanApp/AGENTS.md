...
**⚠️ 报告本身也要提交。** `ACTIVE.md` / `CC-REPORT.md` 的改动和代码一样，
留在工作区就不算完成 —— 验收写的是 `git status --short` **干净**，
不是「代码提交了就行」。

2026-08-31 有两轮是代码提交了、报告留在工作区，而且报告里贴的
`git status --short` 原始输出显示的正是那两份文件未提交 ——
**贴了输出但没看输出**。贴原始输出的意义是让你自己先看一眼。
