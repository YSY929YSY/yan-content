 ...
 * 第 n 个放上去的元素落在哪。
 *
 * ⚠️ **不能都落在正中。** 第一版每个新元素都是 `x:0.5, y:0.5, w:0.42` ——
 * 第二张严丝合缝盖在第一张上面,用户的反馈是「只能上传一张」。
 * 他其实传上去了,只是看不出来。**一个看不见结果的操作等于没做。**
 *
 * 做法是每张错开一点、给一点角度,像随手往桌上放第二张照片。
 * 这**不是替用户排版**(那是原则②禁止的)—— 它只保证「你放的东西看得见」,
 * 位置照样随便拖。真正的排版是把元素摆成某种构图,这里没有构图。
 *
