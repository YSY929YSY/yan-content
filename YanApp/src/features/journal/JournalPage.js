...
// 那不是别的,就是这个缓存只进不出。
//
// 上限 = **注册的纸样档数**,不是「同时显示几张」。
//
// 第一版设了 3(对开显示 2 张 + 1 个余量),结果比没缓存好不了多少:
// 注册了 4 档纸,用户在换纸开关上来回点,每切一次就淘汰一张、切回来又要重解 ——
// 用户的反馈是「切换手帐时要闪一会儿」,就是这个。
// 缓存的意义是「解过的不再解」,上限低于总数等于承诺了又做不到。
//
// 4 档 × 9.4MB ≈ 38MB 常驻。纸已经按屏幕尺寸出(1200 宽,见 generate-paper.py),
// 不是按打印尺寸 —— 那一步省下来的比这里花掉的多。
// 加纸样时**记得同步加这个数**,不然又会退回上面那个情况。
