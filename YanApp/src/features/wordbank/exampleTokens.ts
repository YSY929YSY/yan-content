 ...
 * 都是 Apache-2.0)。**词典 207 MB 不进包,进包的只有派生出来的读音**,509 KB。
 *
...
/** 产物里一个 token 的原始形状。 */
type RawToken = string | [string, string] | unknown;
...
export function normalizeTokens(raw: unknown): ExampleToken[] {
  ...
  for (const t of raw as RawToken[]) {
    if (typeof t === 'string') {
      ...
      out.push(reading ? { text: t[0], reading } : { text: t[0] });
    }
